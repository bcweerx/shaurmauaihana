...
await test('UAH label is stable across server and browser ICU versions', () => {
  if (business.currency === 'UAH' && business.locale === 'uk-UA') {
    assert.equal(formatMoney(420.75), '420,75\u00a0грн');
    assert.equal(formatMoney(0), '0,00\u00a0грн');
  }
});
...
await test('total uses exact minor units and order summary includes every line', () => {
  ...
  const expected =
    (Math.round(a.price * 100) * 3 + Math.round(b.price * 100) * 2) / 100;
...
await test('unconfigured and malformed order channels never create fake links', () => {
  assert.equal(
    orderDestination({ ...business, orderChannel: 'none' }, ''),
    null,
  );
  assert.equal(
    orderDestination(
      { ...business, orderChannel: 'telegram', telegram: 'https://bad' },
      '',
    ),
    null,
  );
  assert.equal(
    orderDestination({ ...business, orderChannel: 'phone', phone: '123' }, ''),
    null,
  );
  assert.equal(
    orderDestination(
      { ...business, orderChannel: 'whatsapp', whatsapp: null },
      '',
    ),
    null,
  );
...
await test('configured channels produce safe URLs without sending an order', () => {
  ...
  const wa = orderDestination(
    { ...business, orderChannel: 'whatsapp', whatsapp: '+380 (00) 000-00-00' },
    text,
  );
  ...
  assert.equal(
    orderDestination(
      { ...business, orderChannel: 'telegram', telegram: 'test_shop' },
      text,
    )?.url,
    'https://t.me/test_shop',
  );
  assert.equal(
    orderDestination(
      { ...business, orderChannel: 'phone', phone: '+380000000000' },
      text,
    )?.url,
    'tel:+380000000000',
  );
  ...
  for (const product of products) {
    ...
    assert.ok(
      product.categories.length &&
        product.categories.every((c) => categories.includes(c)),
    );
    if (product.image)
      assert.ok(existsSync(`public${product.image}`), product.image);
  }
});

await test('page prerenders without browser globals and has meaningful HTML', () => {
  const html = render();
  assert.ok(html.includes('id="menu-title"'));
  assert.ok(html.includes(products[0].name));
